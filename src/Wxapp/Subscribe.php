...
class Subscribe extends Base
{
    ...
    public function getPubTemplateKeyWordsById($tid){
        ...
        $url = 'https://api.weixin.qq.com/wxaapi/newtmpl/getpubtemplatekeywords?access_token='.$token.'&tid='.$tid;
        $res = $this->curl_get($url);
        $res = json_decode($res,true);
        if($res['errcode'] == 0){
            return [
    ...
    public function getPubTemplateTitleList($ids,$start,$limit = 30){
        ...
        $url = 'https://api.weixin.qq.com/wxaapi/newtmpl/getpubtemplatetitles?access_token='.$token.'&ids='.$ids.'&start='.$start.'&limit=30';
